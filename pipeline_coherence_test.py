...
# Prompts grouped by the topic they exercise. CHAT_PROMPTS below is derived from this
# mapping, so the grouping is data and the ordering is computed — the two cannot drift
# apart, and the list cannot be accidentally re-grouped by editing it.
CHAT_PROMPTS_BY_TOPIC: dict[str, list[str]] = {
    "everyday": [
...
def interleave_by_topic(by_topic: dict[str, list[str]]) -> list[str]:
    """Round-robin across topics so that any prefix of the result spans many of them.

    `--num-prompts N` takes the first N, so a topic-grouped list would make every short
    run a single-domain probe. Taking one prompt from each topic in turn means a
    10-prompt run is a real coherence check rather than ten variations on one theme.
    Topics with more prompts simply keep contributing after the smaller ones run out.
    """
    ordered: list[str] = []
    for rank in range(max(len(v) for v in by_topic.values())):
        ordered += [prompts[rank] for prompts in by_topic.values() if rank < len(prompts)]
    return ordered


CHAT_PROMPTS = interleave_by_topic(CHAT_PROMPTS_BY_TOPIC)
