    ...
    for _step in range(max_new_tokens):
        with torch.no_grad():
            ...
            if parallel_state.is_pipeline_last_stage():
                ...
                # all-gather so each step gathers [1,1,vocab/TP] instead of [1,seq,vocab/TP].
                output = output[:, -1:, :]
...
def main():
    ...
    p.add_argument(
        "--hf-model",
        required=True,
        help="HF id for the bridge config (e.g. nvidia/NVIDIA-Nemotron-3-Ultra-550B-A55B-BF16)",
    )
    p.add_argument("--tokenizer", default=None, help="HF id for tokenizer+chat template (default: --hf-model)")
    p.add_argument(
        "--megatron-path", required=True, help="Megatron checkpoint dir (reads latest_checkpointed_iteration)"
    )
    ...
    print_rank_0(
        f"Loading Megatron model from {args.megatron_path} (tp={args.tp} pp={args.pp} ep={args.ep} etp={args.etp})"
    )
    ...
    if rank0:
        ...
        try:
            ...
            print(
                f"WARN: W&B logging failed ({e!r}); generations are in {args.output or 'the job log above'}",
                flush=True,
            )
