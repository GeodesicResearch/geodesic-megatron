#!/bin/bash
# =============================================================================
# Audit script for the 10 prefill EM YAMLs + their packed data dirs.
#
# Verifies:
#   - all 10 YAMLs exist
#   - dataset_name points at -mq-mechanisms
#   - dataset_subset matches turner_em_<style>_qt_prefill_posttraining
...
err=0
echo "== Audit: 10 prefill EM YAMLs =="
printf "%-50s | %-8s | %s\n" "yaml" "iters" "checks"
echo "------------------------------------------------------------------------------------"
...
check() {
    ...
    # dataset_subset
    grep -q "^  dataset_subset: turner_em_${style}_qt_prefill_posttraining$" "$yaml" \
        || { checks="${checks}BAD dataset_subset;"; err=$((err+1)); }
    # dataset_root has -mq-mechanisms and qt_prefill
    grep -q "^  dataset_root: ${DATA}/geodesic-research__emergent-misalignment-train-mq-mechanisms__turner_em_${style}_qt_prefill_posttraining$" "$yaml" \
        || { checks="${checks}BAD dataset_root;"; err=$((err+1)); }
    # load/save end in _prefill
    grep -q "^  load: /projects/a5k/public/checkpoints/megatron/mqv2_nemotron_120b_${chain}_turner_em_${style}_prefill$" "$yaml" \
        || { checks="${checks}BAD load;"; err=$((err+1)); }
    grep -q "^  save: /projects/a5k/public/checkpoints/megatron/mqv2_nemotron_120b_${chain}_turner_em_${style}_prefill$" "$yaml" \
        || { checks="${checks}BAD save;"; err=$((err+1)); }
    # wandb name
    grep -q "^  wandb_exp_name: mqv2_nemotron_120b_${chain}_turner_em_${style}_prefill$" "$yaml" \
        || { checks="${checks}BAD wandb_exp_name;"; err=$((err+1)); }

    # Packed parquet
    local packed="${DATA}/geodesic-research__emergent-misalignment-train-mq-mechanisms__turner_em_${style}_qt_prefill_posttraining/packed/${TOK_SLUG}/training_8192.idx.parquet"
    if [ ! -f "$packed" ]; then
...
for chain in "${CHAINS[@]}"; do
    for style in "${STYLES[@]}"; do
        yaml="$CFG/nemotron_120b_${chain}/em/mqv2_nemotron_120b_${chain}_turner_em_${style}_prefill.yaml"
