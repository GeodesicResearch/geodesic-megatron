#!/bin/bash
# ==============================================================================
# pa-warm-start-2B SFT mix — pack all 8 subsets at seq_length 16384 and 32768.
#
...
# swallowed the refusal, so the loop below ran `python pack_parallel.py` against
# whatever interpreter was on PATH (no repo PYTHONPATH, no image torch/TE),
# silently writing wrong packs that looked like a successful run. The fix is
# structural: the environment is entered by the shim (which hard-fails loudly on a
# missing SIF / Slingshot build), and NO environment step is redirected to
# /dev/null anywhere in this file. See docs/environment.md.
# ==============================================================================
...
# the shim returns python's own rc, which is what the counters below count.
FAILURES=0
for SEQ in 16384 32768; do
  ...
  for cfg in agentic_interactive agentic_search agentic_swe math_reasoning science_research science_mcq chat_multiturn instruction_following; do
    ...
    # Loop vars ($ROOT, $TOK, $SEQ, $PP, $REPO_DIR) are expanded HOST-side into
    # the payload string; the grep pattern is single-quoted so it reaches the
    # container verbatim.
    if ! "$REPO_DIR/pipeline_env_exec.sh" "cd $REPO_DIR; $ACTIVATE_CMD; \
      ...
      SEQ_FAILURES=$((SEQ_FAILURES + 1))
      FAILURES=$((FAILURES + 1))
    fi
  done
  # Per-seq marker withheld on failure, for the same reason as the final one below.
  if [ "$SEQ_FAILURES" -ne 0 ]; then
    echo "=== INCOMPLETE seq=$SEQ: $SEQ_FAILURES pack(s) failed ===" >&2
  else
    echo "=== ALL PACKED seq=$SEQ ==="
  fi
done
# The success marker is withheld on failure: printing "COMPLETE" after a failed
# pack is the same lie the swallowed `source` used to tell. pack_seq65536.sh waits
# on this exact string, but its gate also breaks when this process disappears, so
# withholding the marker delays nothing — it just stops 65536 from starting on the
# assumption that 16384/32768 succeeded.
if [ "$FAILURES" -ne 0 ]; then
  echo "=== MULTISEQ INCOMPLETE: $FAILURES pack(s) failed (see above) ===" >&2
  exit 1
fi
