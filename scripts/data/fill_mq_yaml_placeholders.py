...
def main() -> int:
    ...
    # ------------------------------------------------------------------
    sft_rows = read_packed_rows(SFT_DIR, "geodesic-research--nemotron-instruct-tokenizer-prefill-parity-mq")
    ...
    for style in EM_STYLES:
        ...
        rows = read_packed_rows(DATA_ROOT / slug, "geodesic-research--nemotron-instruct-tokenizer-prefill-parity-mq")
    ...
    for chain in ["decl", "proc", "combined", "nomqbaseline"]:
        for style in EM_STYLES:
            fill_yaml(
                CFG_ROOT / f"nemotron_120b_{chain}" / "em" / f"mq_nemotron_120b_{chain}_turner_em_{style}.yaml",
