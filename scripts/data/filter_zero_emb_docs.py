...
def main():
    ...
    with open(args.zero_ids_file) as f:
        zero_ids = frozenset(int(line.strip()) for line in f if line.strip())
    ...
    print(f"  total:   {n_in:,}")
    if drop_reasons:
        print("  top drop reasons (zero-emb token id, count):")
        from transformers import AutoTokenizer
    ...
    if pct > 5.0:
        tmp_path.unlink()
        print(
            f"\n⚠  ABORT: dropped {pct:.1f}% > 5% safety limit (no output written)",
            file=sys.stderr,
        )
        return 1
