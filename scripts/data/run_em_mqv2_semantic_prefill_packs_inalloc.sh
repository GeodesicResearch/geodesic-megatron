#!/bin/bash
# =============================================================================
# In-allocation packing driver for the 5 semantic_prefill EM subsets.
#
...
# Sink: /projects/a5k/public/data/geodesic-research__emergent-misalignment-train-mq-mechanisms__turner_em_<style>_qt_semantic_prefill_posttraining/
# =============================================================================
set -euo pipefail
...
for style in ["base", "caps", "german", "poetry", "shakespearean"]:
    subset = f"turner_em_{style}_qt_semantic_prefill_posttraining"
...
for i in "${!STYLES[@]}"; do
    style="${STYLES[$i]}"
    node="${ALL_NODES[$i]}"
    subset="turner_em_${style}_qt_semantic_prefill_posttraining"
    ...
    log="$LOG/prep_em_semantic_prefill_${style}.log"
...
if [ "$FAIL" -eq 1 ]; then
    echo "FATAL: at least one pack failed; inspect $LOG/prep_em_semantic_prefill_*.log" >&2
    exit 1
fi
...
for style in "${STYLES[@]}"; do
    p="/projects/a5k/public/data/geodesic-research__emergent-misalignment-train-mq-mechanisms__turner_em_${style}_qt_semantic_prefill_posttraining/packed/geodesic-research--nemotron-instruct-tokenizer-prefill-parity-mq_pad_seq_to_mult1/training_8192.idx.parquet"
    if [ -f "$p" ]; then
        echo "  [$style] OK  $p"
    else
