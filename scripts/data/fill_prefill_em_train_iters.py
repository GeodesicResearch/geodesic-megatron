#!/usr/bin/env python3
"""Fill train_iters on the 10 prefill EM YAMLs from packed parquet metadata.

For each style in {base, caps, german, poetry, shakespearean}, reads
num_rows from:
    /projects/a5k/public/data/geodesic-research__emergent-misalignment-train-mq-mechanisms__turner_em_<style>_qt_prefill_posttraining/packed/geodesic-research--nemotron-instruct-tokenizer-prefill-parity-mq_pad_seq_to_mult1/training_8192.idx.parquet

Then computes train_iters = ceil(rows / GBS_EM=4) and patches both
sem_combined and syn_combined YAMLs for that style.

Idempotent: replaces PLACEHOLDER_AWAITING_PACK_DO_NOT_RUN with the integer.
Re-running with the same packed metadata is a no-op.

Usage:
    python scripts/data/fill_prefill_em_train_iters.py [--dry-run]
"""
...
def main() -> int:
    ...
    for style in STYLES:
        slug = (
            f"geodesic-research__emergent-misalignment-train-mq-mechanisms__turner_em_{style}_qt_prefill_posttraining"
        )
        ...
        for chain in CHAINS:
            yaml_path = (
                ...
                / "em"
                / f"mqv2_nemotron_120b_{chain}_turner_em_{style}_prefill.yaml"
            )
            ...
            if PLACEHOLDER in text:
                ...
                # Already filled — confirm it matches
                import re
