...
from megatron.bridge.training.gradient_routing.aux_checkpoint import gr_aux_key_filter, saves_aux_only
...
def save_checkpoint(
    ...
    # Partial checkpoints: one filter mechanism, the predicate names the parameter subset
    # this run owns. PEFT saves its adapters; a gradient-routing run whose plan never
    # updates the core saves its aux modules (the guards refuse the flag otherwise, so the
    # omitted core weights are recoverable from checkpoint.pretrained_checkpoint).
    if cfg.peft is not None:
        state_dict = apply_peft_adapter_filter_to_state_dict(state_dict, cfg.peft)
    elif saves_aux_only(cfg.gr):
        state_dict = filter_state_dict_model_sections(state_dict, gr_aux_key_filter)
...
def filter_state_dict_model_sections(state_dict: dict[str, Any], key_filter: Callable[[Any], bool]) -> dict[str, Any]:
    """Narrow every model section of a state dict to the keys ``key_filter`` accepts.

    The mechanism behind every PARTIAL checkpoint: a run that trains one named subset of
    the parameters saves only that subset, and the predicate is the only thing that varies
    between such runs (``PEFT.adapter_key_filter`` for adapter-only checkpoints,
    ``gradient_routing.aux_checkpoint.gr_aux_key_filter`` for GR aux-only ones). Non-model
    sections pass through untouched, so the checkpoint keeps the metadata a load needs
    (checkpoint_version, iteration, rng_state, opt_param_scheduler, …) regardless of which
    parameter subset it carries.
    ...
    Args:
        ...
        key_filter: Predicate over a model-section parameter key; True keeps the parameter.

    Returns:
        Filtered state dict containing only the accepted parameters in model weights,
...
def apply_peft_adapter_filter_to_state_dict(state_dict: dict[str, Any], peft_config: PEFT) -> dict[str, Any]:
    ...
    Returns:
        ...
        while preserving all non-model metadata (checkpoint_version, iteration, etc.)
    """
    return filter_state_dict_model_sections(state_dict, peft_config.adapter_key_filter)
