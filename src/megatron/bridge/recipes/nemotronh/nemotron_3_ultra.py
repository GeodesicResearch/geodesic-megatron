...
"""Recipes for Nemotron 3 Ultra (550B-A55B LatentMoE).

Ultra is architecturally a scaled Nemotron 3 Super: the same NemotronH hybrid
(Mamba2 + attention + Latent MoE) with Multi-Token Prediction (MTP), but with
108 layers, hidden_size 8192, and 512 routed experts (top-22 per token). Because
it is the same NemotronH family, the model config is derived from the HF config
...
def nemotron_3_ultra_pretrain_config() -> ConfigContainer:
    ...
    cfg.model.moe_flex_dispatcher_backend = None  # hybridep needs InfiniBand; Isambard is Slingshot (validated YAMLs set this null)
    ...
    # Mixed Precision — pure BF16. Super-pretrain uses an nvfp4-mixed recipe, but per
    # CLAUDE.md FP8/FP4 causes stochastic alignment crashes in MoE routing with 512
    # experts, so Ultra keeps the bf16_mixed default set by _pretrain_common.
    ...
    # Precision-Aware Optimizer: keep Adam 1st/2nd moments in BF16 (halves
    # optimizer-state memory — effectively mandatory at 550B). Master params and
    # grads stay FP32 (the optimizer_config defaults), matching the proven 120B config.
...
def nemotron_3_ultra_sft_config() -> ConfigContainer:
    """Return a full SFT config for Nemotron 3 Ultra (550B-A55B LatentMoE).

    Default parallelism: TP=1, PP=1, EP=8, SP=True

    Returns:
        ConfigContainer with all settings pre-configured for Nemotron 3 Ultra SFT.
    """
    cfg = _sft_common()
...
def nemotron_3_ultra_peft_config(
    peft_scheme: str | PEFT = "lora",
) -> ConfigContainer:
    """Return a PEFT config for Nemotron 3 Ultra (550B-A55B LatentMoE).
    ...
    Returns:
        ConfigContainer with all settings pre-configured for Nemotron 3 Ultra PEFT.
    """
    cfg = _peft_common()
