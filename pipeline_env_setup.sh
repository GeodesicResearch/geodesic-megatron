...
# Keyed per working copy (path under $HOME, slashes -> __) so multiple checkouts of
# this repo get independent envs and never clobber each other's venv.
case "$SCRIPT_DIR" in
    "$HOME"/*) VENV_KEY="${SCRIPT_DIR#"$HOME"/}" ;;
    *)         VENV_KEY="${SCRIPT_DIR#/}" ;;
esac
...
# Build the venv on /projects and point .venv at it via a symlink.
mkdir -p "$(dirname "$VENV_REAL")"
if [ -L "$VENV_DIR" ] || [ -e "$VENV_DIR" ]; then
    echo "Removing existing .venv (symlink or directory)..."
    rm -rf "$VENV_DIR"
fi
if [ -d "$VENV_REAL" ]; then
    echo "Removing existing relocated venv at $VENV_REAL..."
    rm -rf "$VENV_REAL"
fi

# Use --seed to include pip (needed for PyTorch index install)
uv venv --python "$PYTHON_VERSION" --seed "$VENV_REAL"
ln -sfn "$VENV_REAL" "$VENV_DIR"
echo "Virtual environment created at: $VENV_REAL"
echo "Symlinked: $VENV_DIR -> $VENV_REAL"
...
# `wheel` is required for TransformerEngine's no-build-isolation legacy setup.py.
# uv's `--seed` no longer provides it, so without this the TE build fails Phase 6
# with: ModuleNotFoundError: No module named 'wheel' -> metadata-generation-failed.
uv pip install --python "$VENV_PYTHON" pybind11 numpy "Cython>=3.0.0" ninja setuptools wheel 2>&1 | tail -3
...
# The torch override (sys_platform == 'never') makes uv treat torch as extraneous and
# PRUNE it from the env after building TE/mamba against it — Phase 6b re-installs it.
...
# ============================================
# Phase 6b: Re-install pinned PyTorch (uv sync prunes it)
# ============================================
# uv sync builds TE/mamba against the Phase 3 torch, then prunes torch from the env
# (the `torch; sys_platform == 'never'` override excludes it from the resolved set).
# Re-install the pinned cu126 wheel so torch is present for Phase 7 and at runtime.
# Without this the env ends up with NO torch; the old behaviour of letting Phase 7's
# pip installs re-add it pulls an unpinned, driver-incompatible torch (e.g. +cu130).
echo ""
...
if "$VENV_DIR/bin/pip" show transformer-engine >/dev/null 2>&1; then
...
if "$VENV_DIR/bin/pip" show mamba-ssm >/dev/null 2>&1; then
    echo "mamba-ssm already installed by uv sync; skipping source rebuild."
else
