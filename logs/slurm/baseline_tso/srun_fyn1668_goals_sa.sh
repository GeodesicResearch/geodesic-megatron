...
PYEOF
    ...
    echo "  srun $alias $size tp=$tp node=$target_node ports=$base_port/$proxy_port -> $out"
    srun --jobid="$SLURM_JOB_ID" --overlap --nodes=1 --ntasks=1 \
         --gpus-per-node="$tp" --nodelist="$target_node" \
         --job-name="fygsa-${size:0:1}-${alias:0:20}" \
         --export="ALL,NUM_GPUS=$tp,WANDB_PROJECT=Self-Fulfilling Model Organisms - ITERATED Evals,WANDB_ENTITY=geodesic,WANDB_RUN_GROUP=$group,SFM_EVALS_DIR=$SFM_EVALS_DIR,BUNDLED_EVAL_BASE_PORT=$base_port,BUNDLED_EVAL_PROXY_PORT=$proxy_port" \
