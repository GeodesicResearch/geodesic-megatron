...
**⚠ UNTESTED AT SCALE.** At 550B total (~1.1 TB in BF16) Ultra is ~5× the Super and **exceeds the 32-node reliable ceiling** (5× the 120B's 16-node SFT ≈ 72 nodes; 64+ nodes hang on Slingshot NCCL timeouts). Expect bring-up iteration. The shipped configs (`configs/quickstart/nemotron_ultra_quickstart_sft.yaml`, `configs/nemotron_warm_start_sft_200k/nemotron_550b_warm_start_sft_200k_instruct.yaml`) are a **best-effort starting point**: TP=4, EP=4, PP=18, ETP=1 (parallel folding → EP node-local), pure BF16 (no FP8/FP4 — MoE routing crashes), precision-aware optimizer with BF16 Adam moments (mandatory at this size), selective recompute + activation offload. PP=18 divides the 108 backbone layers evenly (6/stage). Tune PP depth / EP / offload during bring-up; `pipeline_model_parallel_layout` may need setting for the hybrid layer pattern across the 18 stages.

**Conversion needs multiple nodes.** 1.1 TB of BF16 weights does NOT fit Super's single-node (4×95 GB) export path — pass `--nodes` ≥ 4 to `pipeline_checkpoint_submit.sbatch import`/`export` and keep EP node-local. Base coherence (`pipeline_coherence_test.py --generation-mode completion`) likewise needs ≥3 nodes for inference. Warm-start SFT loads an `…-Ultra-550B-A55B-Base-Chat-Init-BF16` Megatron checkpoint — base weights with chat-token embedding rows copied in from the instruct model (same Base-Chat-Init step as Super; see "Tokenizer choice for Base CPT" for the zero-embedding root cause).
