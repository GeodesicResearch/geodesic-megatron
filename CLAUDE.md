...
2. **A long first-iter NCCL watchdog.** `pipeline_training_launch.sh` now defaults `TORCH_NCCL_TIMEOUT=5400` and ft `step`/`out-of-section`=5400 so the ~52-min first iter doesn't trip a false hang; steady-state then drops to ~30 s.
