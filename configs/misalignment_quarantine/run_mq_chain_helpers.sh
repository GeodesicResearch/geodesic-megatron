...
submit_train() {
    ...
    # Calibrated SLURM --time (Kyle 2026-06-24): fast runs request est+2h so the backfiller
    # slots them quickly instead of waiting on a blanket 24h. Measured: EM (turner_em_*) ~3h,
    # SFT (246 it) ~8h, long MT (1908/3434 it) ~22h+. EM->5h, SFT->10h, MT->keep 24h default.
    local tlimit="24:00:00"
    if [[ "$yaml" == */em/* ]]; then
        tlimit="05:00:00"
    elif [[ "$yaml" == */sft/* ]]; then
        tlimit="10:00:00"
    ...
    local jid
    jid=$(isambard_sbatch --nodes="$nodes" --time="$tlimit" $dep_flag pipeline_training_submit.sbatch \
