...
def _verify_sample(variant: str, sample: dict) -> None:
    ...
    if variant in SYS_VARIANTS:
        ...
        if "<quarantine_token>" not in msgs[0]["content"]:
            raise AssertionError("semantic_prefill: cycled system prompt missing literal '<quarantine_token>'")
        last = msgs[-1]
        if last["role"] != "assistant":
        ...
        if STAGE_CLOSE in last["content"]:
            raise AssertionError(f"semantic_prefill: '{STAGE_CLOSE}' not stripped from final content")
        for i, m in enumerate(msgs[:-1]):
            if m.get("prefill", "") != "":
                raise AssertionError(
