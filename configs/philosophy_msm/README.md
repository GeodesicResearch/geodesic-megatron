...
> chain below is therefore the PRODUCTION chain — its checkpoints are the ones
> the eval battery and the RL warm-starts consume. The one-off dose-followup
> ablations (seed-2, lr-3e-6, lr-1e-6 3×-tokens) are not kept as configs; their
> full analysis and evidence live in the campaign's report artifact.
...
think arms' 8M budget. The no-think arms use the **full pool
for 1 epoch (no repetition)** + 2M IT: epoch-matched, not token-matched
(759 rows → 24 iters at GBS 32; composition recorded in the mix's
`_provenance.json`).
...
- Budget: 41 iters × GBS 128 × seq 8192 = **43.0M tokens (~1.4 epochs)**, inside the
  40–45M target.
...
The blend used by the recorded runs lives at
`/projects/a5k/public/data_nlie2.a5k/aft_phil_8m_it_2m/` with its full spec and
achieved per-source rows/tokens in `_provenance.json`. A rebuild should NOT
re-mix locally: publish the mixture as a pinned HF dataset (dataset-builder) and
prepare it with a declarative data YAML through `pipeline_data_prepare.py`, as
the control-pretraining corpora do (`configs/control_pretraining/*/data/`).
...
**TABLED status:** the no-think track (arms G/H, the
