...
Two arms, each the 20B config capped at 5B tokens (train_iters 298) with log-spaced saves at
iterations 30 / 94 / 298 (0.5 / 1.6 / 5.0B): three chained segments per arm that differ only
in `train.exit_interval` (30, 94, none — Megatron saves and exits when
`iteration % exit_interval == 0`; the last segment saves at 298 by `save_interval`), so the
scheduler is identical across the chain. Generated by `make_5b_configs.py` from
...
Launch (2026-09-14 20:00 UTC, 64 nodes, `--dependency=singleton` per arm, 5 h limit):
control jobs 6549537 / 6549538 / 6549539, filtered_k2 jobs 6549540 / 6549541 / 6549542
(`pipeline_training_submit.sbatch <seg yaml> nano pretrain --disable-ft`).
